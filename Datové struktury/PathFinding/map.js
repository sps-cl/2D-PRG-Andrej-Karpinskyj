class PathMap {
    ...
    #redrawMap() {
        this.context.clearRect(0, 0, this.canvas.width, this.canvas.height); // Vymaže existující obsah plátna.
        this.#drawLayout(); // Vykreslí základní rozložení.
        // Prochází všechny buňky a vykreslí zdi.
        for (let x = 0; x < this.cells.length; x++) {
            for (let y = 0; y < this.cells[x].length; y++){
                if(this.cells[x][y].wall) this.#drawCell(this.cells[x][y], "black");
            }
        }
        // Vykreslí startovní buňku červeně.
        this.#drawCell(this.start, 'red');
        // Vykreslí cílovou buňku zeleně.
        this.#drawCell(this.target, 'green');
    }

    findPath() {
        //2. upravte tuto metodu takovým způsobem, aby místo pole (cellListu) používala strukturu Binary Heap
        let cellHeap = new BinaryHeap(this.width * this.height);
        cellHeap.add(this.start); // Přidání startovní buňky do haldy
        let closedCells = new Set();
        while(cellHeap.count > 0) {
            let bestCell = cellHeap.get(); // Vyjmutí nejlepší buňky z haldy
            closedCells.add(bestCell); // Přidání vyjmuté buňky do množiny uzavřených buněk
            
            if (bestCell === this.target) {
            return bestCell; } // Pokud je vyjmutá buňka cílová buňka, vrátíme ji
            
            let neighbours = this.getNeighbours(bestCell); // Procházení sousedních buněk
            for (let i = 0; i < neighbours.length; i++) { 
                const neighbour = neighbours[i];
                if (closedCells.has(neighbour)) continue; // Pokud je sousední buňka uzavřená, přeskočíme ji
                let inList = cellHeap.contains(neighbour); 
                let startDistance = bestCell.startDistance + this.getDistance(neighbour, bestCell);
                if (!inList || startDistance < neighbour.startDistance) { // Pokud sousední buňka není v haldě nebo je nová vzdálenost menší než ta předchozí
                    neighbour.startDistance = startDistance; // Aktualizace vzdálenosti od startovní buňky k sousední buňce
                    neighbour.endDistance = this.getDistance(neighbour, this.target); // Vypočítání vzdálenosti od sousední buňky k cílové buňce
                    neighbour.next = bestCell // Nastavení odkazu na předchozí buňku
                    cellHeap.add(neighbour); // Přidání sousední buňky do haldy
                }
            }
        }
    }
    ...
    drawPath() {
        ...
        let path = this.findPath(); // Najde cestu pomocí metody findPath()
        if(!path) return false; // Pokud cesta není nalezena, vrátí false
        while (path.next != this.start) { // Projde postupně jednotlivé buňky a vykreslí je žlutou barvou
            path = path.next;
            this.#drawCell(path, "yellow");
        }
        return true; // Vrátí true jako úspěšně vykreslení cest
    }
